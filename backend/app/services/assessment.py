...
def rebuild(db:Session,user_id:int,concept_id:int):
 ...
 # Preserve gap history while ensuring only the latest evidence remains active.
 db.query(ConceptGap).filter(ConceptGap.user_id==user_id,ConceptGap.concept_id==concept_id,ConceptGap.resolved_at==None).update({'resolved_at':datetime.utcnow()})
