...
export default function Tutor(){
 const [messages,setMessages]=useState<Message[]>([welcome]);const [text,setText]=useState('');const [busy,setBusy]=useState(false);const [error,setError]=useState('');const [conceptId,setConceptId]=useState(1);const [concept,setConcept]=useState('your current concept');const [mode,setMode]=useState<'real'|'demo'>();
 useEffect(()=>{api<DashboardData>('/api/student/dashboard').then(d=>{const target=d.gaps[0]||d.assessments[0];if(target){setConceptId(target.concept_id);setConcept('concept_name' in target?target.concept_name:'your current concept')}}).catch(()=>{})},[]);
