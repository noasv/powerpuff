...
export interface Assessment{concept_id:number;concept_name:string;accuracy:number;average_confidence:number;recall_score:number;transfer_score:number;explanation_score:number;performance:number;calibration_gap:number;concept_mastery:number;risk_level:string;updated_at?:string}
...
export interface Recommendation{concept_id:number|null;concept_name:string|null;subject_name:string|null;gap_id:number|null;action_type:'tutor_remediation'|'assessment';intervention:string;recommended_action:string;reason:string;priority_score:number}
